...
$consulta_datos = "SELECT e.nombre_estado,
COUNT(dord.id_estado) AS total_ordenes,
ROUND(COUNT(dord.id_estado) * 100.0 / (SELECT COUNT(*) FROM detalle_orden WHERE id_turno = $id), 2) AS porcentaje_total
FROM detalle_orden dord
JOIN 
turno_trabajo tu ON dord.id_turno = tu.id_turno
JOIN estado_ot e ON dord.id_estado = e.id_estado
WHERE dord.id_turno = $id
GROUP BY dord.id_estado;;
";
