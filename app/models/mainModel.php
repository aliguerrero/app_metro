...
'<', '>' , '==' , '=' , ';' , '::' ]; $cadena=trim( $cadena ); $cadena=stripslashes( $cadena ); foreach ( $palabras as
    $palabra ) { $cadena=str_ireplace( $palabra, '' , $cadena ); } $cadena=htmlspecialchars( $cadena ); $cadena=trim(
    $cadena ); $cadena=stripslashes( $cadena ); return $cadena; } /* Método protegido para verificar datos */ protected
    function verificarDatos( $filtro, $cadena ) { if ( preg_match( '/^' .$filtro."$/", $cadena ) ) { return false; }
    else { return true; } } /* Método protegido para guardar datos en una tabla de la base de datos */ protected
    function guardarDatos( $tabla, $datos ) { $query="INSERT INTO $tabla (" ; $count=0; foreach ( $datos as $clave ) {
    if ( $count>= 1 ) {
    $query .= ',';
    }
