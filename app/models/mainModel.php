...
"--",
"^",
"<", ">" , "==" , "=" , ";" , "::" ]; $cadena=trim($cadena); $cadena=stripslashes($cadena); foreach($palabras as
    $palabra){ $cadena=str_ireplace($palabra, "" , $cadena); } $cadena=htmlspecialchars($cadena); $cadena=trim($cadena);
    $cadena=stripslashes($cadena); return $cadena; } /* Método protegido para verificar datos */ protected function
    verificarDatos($filtro, $cadena){ if (preg_match("/^".$filtro."$/", $cadena)) { return false; } else { return true;
    } } /* Método protegido para guardar datos en una tabla de la base de datos */ protected function
    guardarDatos($tabla, $datos){ $query="INSERT INTO $tabla (" ; $count=0; foreach($datos as $clave){ if ($count>=1) {
    $query.=",";
    }
    ...
    /* Método protegido para eliminar un registro de una tabla en la base de datos
    protected function eliminarRegistro($tabla, $campo, $id){
    $sql = $this->conectar()->prepare("UPDATE $tabla SET $campo='E-'".$id." ,std_reg=0 WHERE $campo = :id");
    $sql->bindParam(":id", $id);
    $sql->execute();
    ...
    $tabla = '<nav aria-label="...">';
        ...
        if ($pagina <= 1) { $tabla .=' 
                ...
                ' ; } $ci=0; for ($i=$pagina; $i <=$numeroPagina; $i++) { if ($ci>= $botones) {
            break;
            }
            if ($pagina == $i) {
            $tabla .= '
